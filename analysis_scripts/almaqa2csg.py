...
# Todd Hunter (NRAO)
# Dirk Petry (ESO)
# $Id: almaqa2csg.py,v 1.10 2020/05/02 19:42:16 dpetry Exp $
#
"""
...
def version(short=False):
    ...
    """
    myversion = "$Id: almaqa2csg.py,v 1.10 2020/05/02 19:42:16 dpetry Exp $"
    if (short):
    ...
    if step in ['calib']:
        ...
        for msName in msNames:
            ...
            if bpassCalTableName == '':
                ...
                bpassCalTableName = []
                stext = doBandpassCalibration(msName, msName1=msName+'.split', iHaveSplitMyScienceSpw=reindexMyScienceSpw, refant=myRefAnt, calTableName=bpassCalTableName, lowSNR=lowSNR, lbc=lbc, phaseDiff=phaseDiff, valueMaps=valueMaps)
...
def doGenerateWVRCalTable(msName, calTableName=[], refant='', smooth=True, doplot=True, remcloud=False, valueMaps={}):
    ...
    if doplot == True:
        ...
        sciSpwInfo = sfsdr.getSpwInfo(msName, caching=True)     # added by Todd on Sep 12, 2012
...
def doBandpassCalibration(msName, msName1='', bpassCalId='', chanAvg=1.0, refant='', iHaveSplitMyScienceSpw=False, 
                          calTableName=[], lowSNR=False, doplot=True, phaseDiff='', solnorm=True, lbc=False, valueMaps={}):
    ...
    spwInfo1 = sfsdr.getSpwInfo(msName, caching=True)
    spwIds1 = sorted(spwInfo1.keys())

    spwInfo2 = sfsdr.getSpwInfo(msName, intent = 'CALIBRATE_PHASE', caching=True)
