...
logprint ("Starting VLA_pipe_startup.py", logfileout='logs/startup.log')
...
def pipeline_save(shelf_filename='pipeline_shelf.restore'):
    ...
    try:
        keys = [k for k in open(pipepath+'VLA_pipe_restore.list').read().split('\n') if k]
    except Exception, e:
...
logprint ("VLA prototype pipeline reduction", 'logs/startup.log')
...
execfile(pipepath+'VLA_functions.py')
execfile(pipepath+'lib_VLApipeutils.py')
...
try:
    ...
    SDM_name=raw_input("Enter SDM/XP1 file name: ")
...
### RETROFIT UPDATE
# XP1s aren't directories, so I'm forcing the user to get it right 
...
logprint ("Finished VLA_pipe_startup.py", logfileout='logs/startup.log')
