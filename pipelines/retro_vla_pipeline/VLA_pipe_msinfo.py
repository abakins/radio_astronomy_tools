...
logprint ("Starting VLA_pipe_msinfo.py", logfileout='logs/msinfo.log')
...
# RETROFIT ADDITION
# Prompt user to enter names of fields associated with different calibrators
tb.open(ms_active + '/FIELD/')
field_names = tb.getcol('NAME')
target = raw_input('Enter target name: ')
gain = raw_input('Enter gain calibrator name: ')
flux = raw_input('Enter flux calibrator name: ')

intent_list = []
for fn in field_names: 
    if target in fn: 
        intent_list.append('OBSERVE_TARGET')
    elif gain in fn: 
        intent_list.append('CALIBRATE_PHASE')
    elif (flux in fn) and (dual == 'y'): 
        intent_list.append('CALIBRATE_BANDPASS,CALIBRATE_DELAY,CALIBRATE_AMPLI')
    elif flux in fn: 
        intent_list.append('CALIBRATE_FLUX')
    elif band in fn: 
        intent_list.append('CALIBRATE_BANDPASS,CALIBRATE_DELAY')
intent_list = np.array(intent_list)
tb.close()
tb.open(ms_active + '/STATE/', nomodify=False)
if len(tb.getcol('OBS_MODE')) != len(field_names): 
    tb.addrows(len(field_names))
tb.putcol('OBS_MODE', intent_list)
tb.putcol('SIG', np.ones(len(intent_list), dtype=bool))
tb.close()
tb.open(ms_active, nomodify=False)
field_ids = tb.getcol('FIELD_ID')
tb.putcol('STATE_ID', field_ids)
tb.close()

### End Retrofit addition
...
while (ii < len(sorted_frequencies)):
    ...
    while (jj < len(sorted_frequencies)):
        ...
        if (fabs(lower_frequency - upper_frequency) < 1.0):
...
logprint ("Finished VLA_pipe_msinfo.py", logfileout='logs/msinfo.log')
