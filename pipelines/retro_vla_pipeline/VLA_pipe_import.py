...
logprint ("Starting VLA_pipe_import.py", logfileout='logs/import.log')
time_list=runtiming('import', 'start')
QA2_import='Pass'

### RETROFIT UPDATE 
# Switched from SDM importing to XP1 importing 
if (os.path.exists(msname) == False):
    logprint ("Creating measurementset", logfileout='logs/import.log')

    default('importvla')
    archivefiles=SDM_name + '.xp1'
    vis=msname + '.prefix'
    importvla()

    # Making sure everything is referenced in terms of J2000
    fixvis(vis=msname + '.prefix', outputvis=msname, refcode='J2000')
    os.system('rm -rf ' + msname + '.prefix')
...
logprint ("Finished VLA_pipe_import.py", logfileout='logs/import.log')
