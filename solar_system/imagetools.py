...
def read_fits_image(hdu, beam_hdu=None):
    ...
    projcoord = (
        x_grid * getattr(u, hdu.header["CUNIT1"]),
        y_grid * getattr(u, hdu.header["CUNIT2"]),
    )
...
def get_horizons_ephemerides(spice_id, obs_id, date, spheroid_radius):
    ...
    rec = spice.georec(
        output_dict["subobserver_longitude_PD"].to(u.rad).value,
        output_dict["subobserver_latitude_PD"].to(u.rad).value,
        0,
        r_e,
        (r_e - r_p) / r_e,
    )
...
def map_sky_projection(
    projcoord, ghe_dict, rotation_sense="prograde", lon_convention="positive_west"
):
    ...
    """
    subobs_pc = (
        ghe_dict["subobserver_longitude_PC"],
        ghe_dict["subobserver_latitude_PC"],
    )
    ...
    C = (
        lr**2
        + mr**2 * (np.sin(phi0) ** 2 + b**2 * np.cos(phi0) ** 2)
        - rad_ang_radius**2
    )
    coefficients = np.vstack([A * np.ones(C.shape).ravel(), B.ravel(), C.ravel()]).T
    # Vectorized roots compute
    d = coefficients[:, 1:-1] ** 2 - 4.0 * coefficients[:, ::2].prod(
        axis=1, keepdims=True
    )
    roots = (
        -0.5
        * (coefficients[:, 1:-1] + [1, -1] * np.emath.sqrt(d))
        / coefficients[:, :1]
    )
    ...
    if lon_convention == "positive_east":
        pc_grid_lons = np.arctan2(np.real(lp), np.real(nps))
    elif lon_convention == "positive_west":
        pc_grid_lons = np.arctan2(-np.real(lp), np.real(nps))
    else:
        raise ValueError("Invalid longitude convention provided")
    ...
    output_dict = {
        "planetocentric_longitude": pc_grid_lons * u.deg,
...
def map_sky_projection_crs(projcoord, ghe_dict):
    ...
    globe = ccrs.Globe(
        ...
        inverse_flattening=1 / flattening,
    )
    ortho = ccrs.Orthographic(
        central_longitude=subobs_pc[0],
        central_latitude=subobs_pc[1],
        azimuth=np_angle,
        globe=globe,
    )
...
def solve_image_center(image, model, wcs, synthesized_beam, manual_align=False):
    ...
    out = spo.minimize(
        cost_func, [0, 0], args=args, method="Powell"
    )  # Powell is better than others at searching the space
    xr = int(out.x[0])
    yr = int(out.x[1])
    ...
    if manual_align:
        ...
        while not done:
            ...
            im = plt.pcolormesh(
                l_index, m_index, minus_disk, cmap="viridis", shading="auto"
            )
            plt.colorbar()
            plt.draw()
            plt.pause(0.001)
    ...
    output_dict = {"x_roll": xr, "y_roll": yr, "difference_image": difference_image}
...
def brightness_temperature_stats(jy_beam_image, rfi_dict, ghe_dict, mask=None):
    ...
    average_tb = (
        planck_function(frequency, np.sum(jpp) * km_dist**2 / (np.pi * smajor * sminor))
        + 2.726
    )
...
def spatial_binning(
    image, wcs, synthesized_beam, msp_dict, bin_resolution=1, cutoff_angle=80.0
):
    ...
    binned_array = np.vstack(
        [
            ...
            np.degrees(np.arccos(np.array(min_cosemang))),
        ]
    ).T
    ...
    stats_frame["Center latitude"] = (
        pd.cut(stats_frame["Center latitude"], lat_grid)
        .apply(lambda x: x.mid.astype(float))
        .astype(float)
    )
    stats_frame["Center angle"] = (
        pd.cut(stats_frame["Center angle"], angle_grid)
        .apply(lambda x: x.mid.astype(float))
        .astype(float)
    )
    sfgb = stats_frame.groupby(["Center latitude", "Center angle"]).mean()
    sfgb_beamcount = stats_frame.groupby(
        ["Center latitude", "Center angle"]
    ).count() / (np.sum(np.ravel(beam_mask)))
    ...
    sfgb_beamcount = sfgb_beamcount.set_index(
        ["Center latitude", "Center angle"]
    ).dropna()

    # Compute the equivalent image latitude/emission angle resolution and error-per-beam
    _, _, std = sigma_clipped_stats(
        image, sigma=3.0, maxiters=20
    )  # Usually works well for planets...
    sfgb["Beam count"] = sfgb_beamcount[
        "Median image"
    ]  # Its a count, can be any key...
    sfgb["Error estimate"] = np.sqrt(std**2 / sfgb["Beam count"])
    sfgb_hires = sfgb.copy()  # Save the high-res for later

    delta_lat_res = np.round(
        np.nanmedian((sfgb["Max latitude"] - sfgb["Min latitude"]).values.ravel())
    ).astype(int)
    delta_angle_res = np.round(
        np.nanmedian((sfgb["Max angle"] - sfgb["Min angle"]).values.ravel())
    ).astype(int)
...
def fit_binned_limbdarken(
    stats_frame, min_angle_range=10.0, plots=False, use_errors=True
):
    ...
    for i, ind in enumerate(tqdm(data_frame.index)):
        ...
        if len(intens) > np.ceil(min_angle_range / angle_res).astype(int):
            ...
            if plots:
                axs[0].plot(
                    angles, limb_dark_cost(angles, out[0], out[1]), color=angcolors[i]
                )
                axs[0].plot(angles, intens, color=ldcolors[i])
                axs[1].plot(
                    angles,
                    intens - limb_dark_cost(angles, out[0], out[1]),
                    color=angcolors[i],
                )
        else:
...
def fit_binned_limbdarken_exps(
    stats_frame, n_exps=2, min_angle_range=10.0, plots=False, use_errors=True
):
    ...
    for i, ind in enumerate(tqdm(data_frame.index)):
        ...
        if len(intens) > np.ceil(min_angle_range / angle_res).astype(int):
            ...
            pguess = np.insert(
                pguess, 0, np.nanmax(data_frame.values)
            )  # Seed a rough estimate of the nadir brightness
            bnds = [(0, np.inf)]
            ...
            nadir_list.append(float(limb_dark_cost(0.0, *out)))
            jac = spo.approx_fprime(out, ldwrap, np.float64(1e-4), 0.0)
            ...
            if plots:
                ...
                axs[0].plot(angles, intens, color=ldcolors[i])
                axs[1].plot(
                    angles, intens - limb_dark_cost(angles, *out), color=angcolors[i]
                )
        else:
...
def plot_planet(image, header, beam, ghe_dict=None, msp_dict=None):
    ...
    wcs = WCS(header)
    fig, ax = plt.subplots(
        subplot_kw=dict(projection=wcs.celestial), layout="constrained"
    )
    ...
    elps = Ellipse(
        (0.1, 0.1),
        width=beam.major.value,
        height=beam.minor.value,
        angle=beam.pa.value,
        color="white",
        fill=True,
        lw=0.1,
    ...
    if ghe_dict is not None:
        ang_diam = ghe_dict["angular_diameter"]
        b_eff = (
            ghe_dict["apparent_equator_radius"].value
            / ghe_dict["apparent_pole_radius"].value
        )
        np_angle = ghe_dict["north_pole_angle"]
        ...
        # Set limits
        world_xmin = SkyCoord(
            center[0] + ang_diam, center[1] - ang_diam, frame=wcs.wcs.radesys.lower()
        )
        world_xmax = SkyCoord(
            center[0] - ang_diam, center[1] + ang_diam, frame=wcs.wcs.radesys.lower()
        )
    ...
    if msp_dict is not None:
        ...
        CS = ax.contour(
            np.round(grid_lats, 1),
            levels=[0, 30, 45, 60, 75],
            colors="w",
            linewidths=1.0,
        )
        CL = plt.clabel(
            CS, manual=False, fontsize=12, rightside_up=True, use_clabeltext=False
        )
        CS = ax.contour(
            np.round(grid_lons, 1),
...
def resample_image(image, wcs, sample=1.0):
    """Image resampling
    ...
    y2grid, x2grid = np.mgrid[
        : int(image.shape[0] * sample), : int(image.shape[1] * sample)
    ]
    y3grid = y2grid / sample
    x3grid = x2grid / sample

    resample_image = spi.interpn(
        (xgrid[0], ygrid[:, 0]),
        image,
        np.vstack([x3grid.ravel(), y3grid.ravel()]).T,
        bounds_error=False,
        fill_value=f_v,
    )
    ...
    output_dict = {"projection_coordinates": projcoord, "sky_coordinates": skycoord}

    return resample_image, output_dict


def bindown_image(image, wcs, sample=1.0):
    ...
    weights, *_ = np.histogram2d(
        xgrid.ravel(), ygrid.ravel(), bins=(xedges, yedges), weights=image.ravel()
    )
...
def make_zonal_model(lat_bins, disk_intens, ldp, lat_map, emang_map):
    ...
    iot = spi.interp1d(
        ...
        disk_intens[nm],
        bounds_error=False,
        fill_value=(disk_intens[nm][0], disk_intens[nm][-1]),
    )
    pt = spi.interp1d(
        lat_bins[nm], ldp[nm], bounds_error=False, fill_value=(ldp[nm][0], ldp[nm][-1])
    )
    zonedisk = iot(lat_map.ravel()) * np.cos(np.radians(emang_map.ravel())) ** pt(
        lat_map.ravel()
    )
    zonedisk = zonedisk.reshape(emang_map.shape)
    zonedisk[np.isnan(zonedisk)] = 0.0
...
def make_zonal_model_exps(lat_bins, coeffs, lat_map, emang_map):
    ...
    expdisk[np.isnan(expdisk)] = 0.0
    expdisk[expdisk < 0] = 0.0
...
def planck_function(freq, intens):
    ...
    """
    Bv = intens * 1e-26
    log_arg = 2.0 * spc.h * freq**3.0 / (spc.c**2.0 * Bv) + 1.0
